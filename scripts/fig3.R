...
p1 <- aa_df |>
  ...
  scale_y_continuous(labels = abs, limits = \(x) c(-max(abs(x)), max(abs(x)))) + 
  theme_minimal() +
  facet_wrap(vars(airline_airport), scales = "free_y", ncol = 2, dir = "v") +
  scale_fill_manual(name = "Flight type", values = color_list, breaks = c("Arrival", "Departure")) +
  scale_color_manual(name = "Flight type", values = color_list, breaks = c("Arrival", "Departure")) +
...
# # try to add a map but it doesn't look well
# airports <- read_csv('data/airports_in_cont_us.csv')
# airports_aa <- airports |> right_join(airport_df, by = c("airport" = "aa_hubs"))
# us_map_sf <-  us_map(regions = 'states') |> filter(!abbr %in% c("AK", "HI", "PR"))
# p2 <- ggplot() +
#   geom_sf(data = us_map_sf, color = "grey70", fill = "grey90") +
#   geom_point(data = airports_aa, aes(x = x, y = y), color = '#005f86') +
#   geom_label(data = airports_aa, aes(x = x, y = y - 50000, label = airport), color = '#005f86') +
#   theme_void() +
...
ggsave(plot = p1,
       ...
       height = 20, width = 20, unit = "cm", bg = "white")
