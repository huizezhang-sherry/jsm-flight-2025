...
two_df <- flight_df_raw |>
  ...
  summarize_count(airports = c("AUS", "DFW"), block_size = 15) |> 
...
aa_df <- flight_df_raw |>
  ...
  summarize_count(airports = aa_hubs, block_size = 15) |> 
  mutate(type = factor(type, levels = c('dep', 'arr'), labels = c("dep" = 'Departure', "arr" = 'Arrival'))) |> 
