...
ggplot(data = dfDayUsesTotals, aes(x = dailyUses, y = n)) +
  ...
  scale_x_discrete(limits = as.character(seq(1, max(dfDayUsesTotals$dailyUses), 1))) +
...
dfSub <- subset(df, Month == 3 & Reporting_Airline == "WN")
...
dfDayUsesDels <- merge(dfDayUses, dfDayDels)
tabs <- as.data.frame(table(dfDayUsesDels$dailyUses, dfDayUsesDels$FlightDate))
colnames(tabs) <- c("dailyUses", "FlightDate", "freq")
...
dfDayUsesDelsTotals <- merge(dfDayUsesDelsTotals, tabs)
...
ggplot(data = dfDayUsesDelsTotals, aes(x = dailyUses, y = aveDelay)) +
  ...
  scale_x_discrete(limits = as.character(seq(1, max(dfDayUsesDelsTotals$dailyUses), 1))) +
  facet_wrap(~FlightDate) +
  geom_text(aes(x = dailyUses, y = max(aveDelay) + 50, label = as.character(freq)), size = 2) +
  theme_few()
