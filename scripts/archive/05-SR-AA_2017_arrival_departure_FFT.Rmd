---
title: "AA_2017_arrival_departure_FFT"
...
```{r AA airports}
df_aa_airports <- read.csv("pairs20.csv")
aa_airports <- df_aa_airports$origin[which(df_aa_airports$airline  == "AA")]
aa_airports_quartile <- df_aa_airports$quartile[which(df_aa_airports$airline  == "AA")]
print(aa_airports) ; print(aa_airports_quartile)
...
```
## FFT for checking peridicity

```{r FFT 2023, echo = FALSE}
...
quartile_arr <- c() 
for (a in top2_arr$airport){
   quartile_arr <- c(quartile_arr, df_aa_airports$quartile[which(df_aa_airports$origin == a & df_aa_airports$airline == "AA")])
}
quartile_arr <- as.factor(quartile_arr)
top2_arr$quartile <- factor(quartile_arr)

p_arr <- ggplot(top2_arr, aes(x = top1_period_mins, y = top2_period_mins, label = airport, color = quartile)) +
  geom_point(size = 3) +
  geom_text(vjust = -0.6, size = 3.5) +
  scale_x_log10() +
  scale_y_log10() +
  labs(
    title = "Top 2 FFT Periods for Arrivals - AA (2017)",
    x = "Top 1 Period (min)",
    y = "Top 2 Period (min)"
  ) +
  theme_minimal()
...
quartile_dep <- c() 
for (a in top2_dep$airport){
   quartile_dep <- c(quartile_dep, df_aa_airports$quartile[which(df_aa_airports$origin == a & df_aa_airports$airline == "AA")])
}
quartile_dep <- as.factor(quartile_dep)
top2_dep$quartile <- factor(quartile_dep)

p_dep <- ggplot(top2_dep, aes(x = top1_period_mins, y = top2_period_mins, label = airport, color = quartile)) +
  geom_point(size = 3) +
  geom_text(vjust = -0.6, size = 3.5) +
  scale_x_log10() +
  scale_y_log10() +
  labs(
    title = "Top 2 FFT Periods for Departures (AA 2017)",
    x = "Top 1 Period (min)",
    y = "Top 2 Period (min)"
  ) +
  theme_minimal()
...
# Arrival clustering
set.seed(123)
k_arr <- kmeans(arr_features[, -1], centers = 3)
arr_features$cluster <- factor(k_arr$cluster)

# Departure clustering
set.seed(123)
k_dep <- kmeans(dep_features[, -1], centers = 3)
