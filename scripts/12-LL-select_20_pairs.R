...
pv <- subset(pv, subset = UNIQUE_CARRIER %in% c("AA"),
...
flights <- subset(flights, Reporting_Airline %in% c("AA"))
...
colnames(df20) <- c("airline", "origin", "passenger_count", "quartile")
write.csv(df20, "pairs20.csv", row.names = FALSE)
